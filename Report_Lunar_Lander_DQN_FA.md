...
**شاخه مدل منتخب:** `final/double-dqn-buffer100k`
...
| Policy Network و Target Network | دو نمونه از کلاس `DQN` | انجام‌شده |
...
| پاسخ چهار سؤال مفهومی | بخش ۱۳ | انجام‌شده |
...
ازآنجاکه فضای وضعیت Lunar Lander پیوسته است، نگهداری یک جدول Q عملی نیست. به همین دلیل یک شبکه عصبی پارامتری با وزن‌های \(\theta\) برای تقریب \(Q(s,a;\theta)\) استفاده می‌شود.
...
\begin{cases}
r & d=1 \\
r+\gamma\max_{a'}Q_{\text{target}}(s',a') & d=0
\end{cases}
\]
...
\[
\hat{y}=Q_{\text{policy}}(s,a)
\]

سپس اختلاف \(\hat{y}\) و \(y\) با Huber Loss یا `Smooth L1 Loss` کمینه می‌شود:

\[
L(\theta)=\operatorname{Huber}(\hat{y},y)
\]
...
\[
(8\times128+128)+(128\times128+128)+(128\times4+4)=18180
\]
...
\[
\text{Replay Buffer Capacity}=100000
\]
...
a =
\begin{cases}
\text{Random Action} & \text{با احتمال }\epsilon \\
\arg\max_a Q_{\text{policy}}(s,a) & \text{با احتمال }1-\epsilon
\end{cases}
\]
...
\[
\epsilon \leftarrow \max(0.01,0.995\epsilon)
\]
...
در ابتدای آموزش وزن‌های Policy Network در Target Network کپی می‌شوند. سپس هر `1000` مرحله یادگیری، وزن‌ها به‌صورت Hard Update منتقل می‌شوند:
...
## ۱۰. فرآیند آموزش و انتخاب Checkpoint
...
**شکل ۱ —** سمت چپ: پاداش هر اپیزود، Moving Average با پنجره ۵۰ و میانگین Validation. سمت راست: Average Max Q روی مجموعه‌ای ثابت از وضعیت‌ها.
...
## ۱۲. آزمایش‌های کنترل‌شده و انتخاب مدل نهایی
...
اگر همان شبکه‌ای که \(Q(s,a)\) را پیش‌بینی می‌کند هم‌زمان Target را نیز تولید کند، پس از هر Update هم خروجی مورد یادگیری و هم مقدار هدف تغییر می‌کنند. این وضعیت **Moving Target** نام دارد.
...
در پروژه حاضر این انتقال هر `1000` Learning Step انجام می‌شود. ثابت‌ماندن Target Network در فاصله دو Update باعث می‌شود مسئله رگرسیون در بازه‌های کوتاه Target نسبتاً ثابتی داشته باشد و آموزش پایدارتر شود.
...
\[
\epsilon_t=\max(0.01,1.0\times0.995^t)
\]
...
\[
Q(s,a)=\mathbb{E}\left[r_{t+1}+\gamma r_{t+2}+\gamma^2r_{t+3}+\cdots\right]
\]
...
## ۱۵. نقاط قوت، محدودیت‌ها و تهدیدهای اعتبار
...
- [ ] `test.py` در حالت عادی رندر گرافیکی محیط را فعال کند.
...
- [ ] نسخه کامل پژوهشی در Branch `final/double-dqn-buffer100k` حفظ شود.
