...
**درس:** هوش مصنوعی  
**نام اعضای گروه:** آراد ایزدی دوست، احسان رضوی 
**شماره‌های دانشجویی:** 403105758, 403106002   
...
۵. **انتخاب مدل و Holdout نهایی:**  
   مدل Buffer-100k پیش از مشاهده Holdout انتخاب شد. سپس فقط همان مدل روی Seedهای `10000` تا `10099` اجرا شد.

 ۵.۱. Artifactهای اصلی شامل وزن‌ها، `training_metrics.json`، نمودار آموزش، Benchmarkهای بزرگ، گزارش مقایسه و Holdout نهایی در شاخه نهایی پروژه نگهداری شدند. پوشه محلی `runs/` بخشی از ZIP تحویل نیست و صرفاً برای مدیریت آزمایش‌ها استفاده شده است.
...
ریپلای بافر (Replay Buffer) به‌صورت یک حافظه حلقوی پیاده‌سازی شده است. هر تجربه شامل وضعیت، عمل، پاداش، وضعیت بعدی و فلگ پایان اپیزود است. در صورت پرشدن حافظه، تجربه‌های قدیمی به‌ترتیب بازنویسی می‌شوند.
...
- ۱. `policy_net`: شبکه‌ای که با Backpropagation به‌روزرسانی می‌شود.
- ۲. `target_net`: شبکه‌ای که برای محاسبه Targetهای Bellman استفاده می‌شود.
...
۱. محیط Reset می‌شود.
۲. Agent با سیاست `ε-Greedy` عمل را انتخاب می‌کند.
۳.  محیط `next_state`، `reward` و `done` را برمی‌گرداند.
۴.  Transition در Replay Buffer ذخیره می‌شود.
۵. یک Mini-batch تصادفی نمونه‌گیری شده و Policy Network به‌روزرسانی می‌شود.
۶. در صورت رسیدن شمارنده یادگیری به مضرب `1000`، Target Network به‌روزرسانی می‌شود.
۷. پس از پایان اپیزود، Epsilon کاهش می‌یابد.
۸. هر ۵۰ اپیزود، ده اپیزود Validation کاملاً Greedy روی Seedهای `901–910` اجرا می‌شود.
۹. در صورت بهبود میانگین Validation، وزن‌ها در `weights.pth` ذخیره می‌شوند.
...
**شکل ۱ 
** سمت چپ: پاداش هر اپیزود، Moving Average با پنجره ۵۰ و میانگین Validation. 
سمت راست: Average Max Q روی مجموعه‌ای ثابت از وضعیت‌های پایش Q. این مجموعه فقط برای رسم شاخص Average Max Q است و با Holdout نهایی ارزیابی تفاوت دارد.
...
- هارد اپدیت های دوره‌ای Target Network که می‌توانند توزیع Targetها را تغییر دهند؛
...
- بافر صدهزار تایی در `1222` مورد از `1500` اپیزود پاداش بیشتری داشت.
- سافت اپدیت تنها در `278` مورد بهتر بود.
...
بافر بزرگ‌تر باعث حفظ تنوع بیشتر تجربه‌ها شد. در مقابل، Soft Update با وجود میانگین مناسب، رفتارهای طولانی‌تر و Streakهای غیرعادی موتورهای جانبی ایجاد کرد. در Benchmarkهای Soft Update، طولانی‌ترین Streak موتور چپ به `468` و `293` رسید؛ درحالی‌که مقادیر متناظر مدل نهایی بسیار محدودتر بودند. بنابراین Buffer-100k هم از نظر Reward و هم از نظر Reliability و کارایی زمانی انتخاب بهتری بود.
...
هولداوت از Benchmarkهای انتخاب مدل نیز بهتر بود: میانگین از `267.43` به `276.33` و نرخ موفقیت از `95.07%` به `97%` رسید. این نتیجه نشان می‌دهد مدل روی داده‌های انتخاب‌نشده نیز تعمیم مناسبی دارد.
...
ترنزیشن های یک اپیزود به‌شدت به یکدیگر وابسته‌اند. برای مثال، چند وضعیت متوالی هنگام سقوط تقریباً موقعیت، سرعت و زاویه مشابهی دارند. اگر شبکه مستقیماً با همین ترتیب زمانی آموزش ببیند، فرض تقریباً مستقل و هم‌توزیع بودن نمونه‌ها نقض می‌شود.
...
- ۱. Policy Network در هر Gradient Step تغییر می‌کند.
- ۲. Target Network برای مدتی ثابت می‌ماند.
- ۳. هر چند مرحله، وزن Policy در Target کپی می‌شود.
...
- ریپلی بافر تنوع کمتری خواهد داشت.
...
در نتیجه Decay برابر `0.995` در سطح اپیزود، یک تعادل میان اکتشاف اولیه و بهره‌برداری نهایی ایجاد کرد. مقدار حداقل `0.01` نیز مقدار کمی اکتشاف را تا پایان آموزش حفظ می‌کند.
...
- ۱. پیاده‌سازی کامل DQN بدون کتابخانه آماده RL؛
- ۲. جداسازی Policy و Target Network؛
- ۳. Replay Buffer بزرگ و Mini-batch تصادفی؛
- ۴. Validation جدا از Test و Holdout؛
- ۵. انتخاب Checkpoint در حالت Greedy؛
- ۶. مقایسه جفت‌شده روی ۱۵۰۰ Seed یکسان؛
- ۷. نگه‌داشتن Holdout تا پایان فرآیند انتخاب مدل؛
- ۸. عملکرد نهایی قوی با نرخ موفقیت `97%`.

### محدودیت‌ها

- ۱. انتخاب تنظیم نهایی با یک Training Seed انجام شده است؛ بنابراین حساسیت کامل به Seed آموزش اندازه‌گیری نشده است.
- ۲. Validation تنها شامل ده اپیزود است و در میانه آموزش نوسان بالایی دارد.
- ۳. برخی نام‌های داخلی Branch و Metadata عبارت `double_dqn` دارند، درحالی‌که فرمول Target مربوط به DQN استاندارد است.
- ۴. Benchmarkهای بزرگ ارزیابی را دقیق‌تر می‌کنند، اما جای تکرار آموزش با چند Seed مستقل را به‌طور کامل نمی‌گیرند.
- ۵. رفتار تصادفی محیط و Bootstrap می‌توانند موجب افت‌های مقطعی حتی پس از یادگیری سیاست خوب شوند.
